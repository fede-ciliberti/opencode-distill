...
> Estado: Q1/Q4/Q5/Q6 ✅ cerradas (ver evidencia citada); Q2 🔶 abierta con evidencia (SKIP, falta credencial Anthropic); Q3 ✅ cerrada (no aplica, scratch session es la vía).
...
## Q3, ¿`prompt({noReply:true})` evita crear mensajes visibles? ✅ CERRADA (no aplica)

- **Resolución**: el diseño eligió **scratch session** (`/tmp/opencode/distill-<ts>`) como vía del destilador (C3, docs/01 §5.2). Con scratch session, `prompt({noReply:true})` es irrelevante, no hay mensaje del destilador que ocultar en la sesión real.
- No se requiere smoke adicional. Si alguna vez se descarta la scratch session, reabrir con: `prompt` con `noReply:true` y luego `session.messages`, ¿aparece el mensaje del user?
